...
static void _socket_event_cb(HySocketConnectState_e state, void *args)
{
    LOGD("state: %d, args: %p \n", state, args);
}
...
int main(int argc, char *argv[])
{
    ...
    HySocketConnect(context->socket_h, "192.168.1.57", 7890);

    char *buf = "12345";
    while (!context->exit_flag) {
        HYSocketWrite(context->socket_h, buf, HY_STRLEN(buf));
        sleep(1);
    }
